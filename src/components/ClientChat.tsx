...
const ClientChat: React.FC = () => {
  ...
  const [availableRealtors, setAvailableRealtors] = useState<User[]>([]);
  const [showRealtorSearch, setShowRealtorSearch] = useState(false);
  ...
  const fetchAvailableRealtors = async () => {
    try {
      if (!token) return;
      
      const response = await fetch(`${BASE_URL}/api/chat/search-users?user_type=Realtor&q=${searchQuery}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      if (response.ok) {
        const result = await response.json();
        setAvailableRealtors(result.data || []);
      }
    } catch (error) {
      console.error('Failed to fetch available realtors:', error);
    }
  };
  ...
  const startChatWithRealtor = async (realtor: User) => {
    try {
      setLoading(true);
      ...
      const response = await fetch(`${BASE_URL}/api/chat/chats`, {
        ...
        body: JSON.stringify({
          participant_id: realtor.user_id,
          participant_type: 'Realtor'
        })
      });
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setShowRealtorSearch(!showRealtorSearch)}
          ...
          <div className="p-4 border-b border-gray-200 bg-gray-50">
            ...
            <div className="mt-2 max-h-32 overflow-y-auto">
              {availableRealtors.map((realtor) => (
                <div
                  key={realtor.user_id}
                  onClick={() => startChatWithRealtor(realtor)}
        ...
        <div className="flex-1 overflow-y-auto">
          {chatRooms.length === 0 ? (
            <div className="p-4 text-center text-gray-500">
              ...
              <p className="text-sm">Start a chat with a realtor</p>
            </div>
          ) : (
            chatRooms.map((room) => {
        ...
        {selectedChat ? (
          <>
            ...
            <div className="p-4 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <div className="font-medium text-gray-900">
                    {getOtherParticipant(selectedChat)?.name || 'Realtor'}
                  </div>
          ...
          <div className="flex-1 flex items-center justify-center text-gray-500">
            <div className="text-center">
              ...
              <p className="text-sm">or start a new chat with a realtor</p>
            </div>
          </div>
        )}
      </div>
