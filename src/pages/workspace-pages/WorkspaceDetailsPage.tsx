...
export function WorkspaceDetailsPage() {
  ...
  const validateWorkspaceName = (name: string) => {
    ...
    if (invalidChars.test(name)) {
      return { isValid: false, message: 'Workspace name cannot contain special characters like *, +, =, $, %, ^, &, (, ), !, @' };
    }
    
    // Check length
    if (name.length < 3 || name.length > 50) {
      return { isValid: false, message: 'Workspace name must be between 3 and 50 characters' };
    }
  ...
  const checkWorkspaceAvailability = async (name: string) => {
    ...
    if (!validation.isValid) {
      setNameValidation({ isValid: false, isChecking: false, message: validation.message });
      return;
    }

    setNameValidation({ isValid: true, isChecking: true, message: 'Checking availability...' });

    try {
      // Convert name to slug format
      const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
      
      const response = await fetch(`${BASE_URL}/api/workspaces/check-exist?name=${slug}`, {
        method: 'GET',
        credentials: 'include',
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.data?.available) {
          ...
          setNameValidation({ 
            isValid: false, 
            isChecking: false, 
            message: 'This workspace name is already taken. Please choose a different name.' 
          });
        }
      } else {
        setNameValidation({ 
          ...
          <form onSubmit={handleContinue} className="space-y-8">
                ...
                <div className="mt-2 flex items-center space-x-2">
                  {nameValidation.isChecking ? (
                    <>
                      ...
                      <span className="text-sm text-blue-600">{nameValidation.message}</span>
                    </>
                  ) : nameValidation.isValid ? (
                    <>
                      <CheckCircle className="w-4 h-4 text-green-600" />
                      <span className="text-sm text-green-600">{nameValidation.message}</span>
                    </>
                  ) : (
                    <>
                      <AlertCircle className="w-4 h-4 text-red-600" />
                      <span className="text-sm text-red-600">{nameValidation.message}</span>
                    </>
                  )}
                </div>
              )}
              ...
              <div className="space-y-4">
                <div>
                  <label className="cursor-pointer">
                    ...
                    <div className="flex items-center space-x-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                      <Upload className="w-4 h-4 text-gray-500" />
                      <span className="text-sm text-gray-700">Upload Signature Files</span>
                    </div>
                  </label>
                  <p className="text-xs text-gray-500 mt-1">PNG, JPG, PDF files up to 5MB each</p>
                </div>
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Select features you want to disable in this workspace. Disabled features will not be accessible to workspace members.
              </p>
                ...
                <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {disabledFeatures.map(featureId => {
                      ...
                      return (
                        <span key={featureId} className="px-2 py-1 text-xs bg-red-100 text-red-800 rounded-full">
                          {feature?.label}
                        </span>
                      );
                    })}
            ...
            <Button 
              ...
              className="w-full"
              loading={loading}
              disabled={!nameValidation.isValid || nameValidation.isChecking || !workspaceName.trim()}
            >
              Create Organization Workspace
            </Button>
          </form>
